...
import { WARDuinoRuntime, MockBreakpoint } from './warduinoRuntime';
...
export class WarduinoDebugSession extends LoggingDebugSession {
	...
	private _runtime: WARDuinoRuntime
	 ...
	 */
	public constructor(path,port) {
		super("mock-debug.txt");
		// this debugger uses zero-based lines and columns
		this.setDebuggerLinesStartAt1(true);
		this.setDebuggerColumnsStartAt1(true);
		this._runtime = new WARDuinoRuntime(path,port);
		this.setup_handlers();
	}

	public setup_handlers() {
		this._runtime.on('stopOnEntry', () => {
			this.sendEvent(new StoppedEvent('entry', WarduinoDebugSession.THREAD_ID));
		});
		this._runtime.on('stopOnStep', () => {
			this.sendEvent(new StoppedEvent('step', WarduinoDebugSession.THREAD_ID));
		});

		this._runtime.on('fatal', (error) => {
			vscode.window.showErrorMessage(error.message);
			this.sendEvent(new TerminatedEvent());
		});

		this._runtime.on('stopOnBreakpoint', () => {
			this.sendEvent(new StoppedEvent('breakpoint', WarduinoDebugSession.THREAD_ID));
		});
		this._runtime.on('stopOnDataBreakpoint', () => {
			this.sendEvent(new StoppedEvent('data breakpoint', WarduinoDebugSession.THREAD_ID));
		});
		this._runtime.on('stopOnException', () => {
			this.sendEvent(new StoppedEvent('exception', WarduinoDebugSession.THREAD_ID));
		});
	...
	protected threadsRequest(response: DebugProtocol.ThreadsResponse): void {
		...
		response.body = {
			threads: [
				new Thread(WarduinoDebugSession.THREAD_ID, "thread 1")
			]
		};
		this.sendResponse(response);
	}
	...
	protected scopesRequest(response: DebugProtocol.ScopesResponse, args: DebugProtocol.ScopesArguments): void {
		...
		response.body = {
			scopes: [
				...
				new Scope("WARDuino", this._variableHandles.create("warduino"), true)
			]
		};
		this.sendResponse(response);
	}
